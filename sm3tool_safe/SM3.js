...
function sm3(msg) {
  ...
  // pad: append 0x80, then k zero bytes so that (msgBytes.length + 1 + k) % 64 == 56, then append 8-byte big-endian length
  const oneByte = Buffer.from([0x80]);
  const mod = (msgBytes.length + 1) % 64;
  const zeroPadLen = (mod <= 56) ? (56 - mod) : (56 + 64 - mod);
  const zeroPad = Buffer.alloc(zeroPadLen);
  const lenBuf = Buffer.alloc(8);
  lenBuf.writeUInt32BE(Math.floor(bitLen / 0x100000000), 0);
  lenBuf.writeUInt32BE(bitLen >>> 0, 4);
  const padded = Buffer.concat([msgBytes, oneByte, zeroPad, lenBuf]);
